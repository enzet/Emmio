...
class Teacher:
    ...
    def __init__(self, learning_id: str, config: dict, options: dict) -> None:
        ...
        self.user_file_name = user_config['file']
        ...
        self.dictionary = reader.read_dict(dictionary_config['file'],
        ...
        if 'priority' in learning_config:
            ...
            for priority_list_id in priority_list_ids:
                ...
                priority_list = reader.read_priority(priority_config['file'])
    ...
    def run(self):
        ...
        '''
        end_statistics = analysis.get_statistics(user_data, cards, now, postfix)
        end_time = datetime.now()
        print('\n\n' + \
            begin_time.strftime('%H:%M') + "-" + end_time.strftime('%H:%M') + \
            ' learn, _: English, using: Emmio,\n' + \
            '            at_start: {now: ' + \
            str(begin_statistics['to_repeat']) + ', ' + \
            'learned: ' + str(begin_statistics['learned']) + ', ' \
            'not: ' + str(begin_statistics['not_learned']) + ', ' \
            'all: ' + str(begin_statistics['learned'] + \
            begin_statistics['skipped']) + '},\n            at_end: {now: ' + \
            str(end_statistics['to_repeat']) + ', ' + \
            'learned: ' + str(end_statistics['learned']) + ', ' \
            'not: ' + str(end_statistics['not_learned']) + ', ' \
            'all: ' + str(end_statistics['learned'] +
            end_statistics['skipped']) + '}\n')
        '''
    ...
    def write_data(self):
        ...
        open('archive/' + str(now) + '-' + self.user_file_name, 'w+').write(obj)
